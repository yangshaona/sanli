...
//  if(!empty( $st2)){
    //$model1=ClubNews::model()-> get_chose_news($st2->club_id);
       $model1=ClubNews::model()-> get_all_news(0);
  //}
  $xs_sign=ClubNewsSignList::model()-> get_sign_news_late($stsnum);
...
<form action="<?php echo Yii::app()->request->url;?>" method="get">
  ...
  <tr > 
    ...
    <td width="20%" id="STLEVEL" ><?php  if(!empty($st2)) { echo $st2->SCLEV;} ?></td>
    <td width="10%"><?php echo $form->labelEx($model,'STCLASS'); ?></td>
    <td width="20%" id="STCLASS"><?php if(!empty($st2)) { echo $st2->SCCLASS;} ?></td> 
...
<tr > 
      ...
      <td  id="SCSNUM" ><?php if(!empty($st2)) { echo$st2->SCSNUM;} ?></td>
