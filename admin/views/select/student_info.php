<?php 
   ...
   $st2= Sclass::model()->find("STSNUM=".$stsnum.'  order by  SCYEAR desc');
   $cid=0;
   if(!empty($model)){
     $cid=$model->club_id;
   }
   $model1=ClubNews::model()-> get_all_news_year($cid);
   $xs_sign=ClubNewsSignList::model()->get_sign_news($stsnum);
   $model2=ClubList::model()-> find('id='.$cid);
   $tmp_chage=Datechange::model()->change();
?>
...
<ion-scroll  direction="y" scrollbar-y="false" style="width: 100%; height: 100%">
  ...
  <div class="box-table">
   <table class="list" id='list2' >
          <tr > 
            ...
            <td width="15%" id="STLEVEL" ><?php if(isset($st2->SCLEV)) echo $st2->SCLEV; ?></td>
            <td width="8%"><?php echo $model->getAttributeLabel('STCLASS'); ?></td>
            <td width="20%" id="STCLASS"><?php  if(isset($st2->SCLEV)) echo $st2->SCCLASS; ?></td> 
            <td rowspan="4" width="40%" id='stpic' align="center">
        <?php 
         $s1=''.$stsnum;
         $p='http://202.175.81.109/upload/IMG/HKPIC/P'.substr($s1,0,4).'/s'.$s1.'.jpg';
        ?>
  ...
  if (!empty($model2)) { ?> 
   <tr > 
      <td><?php echo $model->getAttributeLabel('STPHONE'); ?></td> 
   
  <?php if(($tmp_chage)||empty( $model->STPHONE)){  ?>
   <td>
      <input type="text" id="STPHONE" style="background-color:#C8C8C8;" value="<?php echo $model->STPHONE;?>">  </td>  <td>
  <a  id="j-save" class="btn" href="javascript:;" onclick="update_contact_phone(<?php echo $stsnum;?>);" >
  <font style="color:#000000;">修改保存</font></a> 
   </td> 
      </tr>
   
 <?php } else { ?>
   <td colspan="2">
 <?php echo $model->STPHONE; } ?>
...
<script>
  function update_contact_phone(pstsnum){
     var s2='<?php echo $this->createUrl("GfUser1/SavePhone");?>';
     var s1=$('#STPHONE').val();

      $.ajax({
        type: 'get',
        url: s2,
        data: {'stsnum':pstsnum,'STPHONE':s1},
        dataType:'json',
